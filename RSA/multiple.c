...
void bignum_frombin(bignum* b, char* message, int len)
{
	b->length = len / sizeof(word);
	b->capacity = b->length;
	b->data = malloc(b->capacity*sizeof(word));
	memcpy_s(b->data, len, message, len);
}
...
void bignum_printhex(FILE* fp, bignum* b)
{
	word w;
	char t[9];
	char c = ' ';
...
//void randPrime(int digits, bignum* result) {
//	int len = digits / 32;
//
//	result->capacity = len;
//	result->length = len;
//	result->data = realloc(result->data,sizeof(word) * len);
//
//	for (int i = 0; i < len; ++i)
//		result->data[i] = (word)rand();
//
//	while (result->data[0] % 2 != 1)
//		result->data[0] = (word)rand();
//
//	while (1) {
//		if (probablePrime(result, ACCURACY)) {
//			return;
...
#ifndef NOPRINT
		//bignum_print(stdout, &encoded[i / bytes]);
		printf("%d,%d:", i/bytes,encoded[i / bytes].length);
		bignum_printhex(stdout, &encoded[i / bytes]);
		printf("\n");
#endif
	}
	return encoded;
...
void encodeToFile(FILE *fp,unsigned char* message,int len,int bytes, bignum* exponent, bignum* modulus) {
	...
	for (i = 0; i < len; i += bytes) {
		...
		encode(x, exponent, modulus, encoded);

		if (encoded->length < modulus->length)
		{
			int lack = modulus->length - encoded->length;
			encoded->data = realloc(encoded->data, modulus->length * sizeof(word));
			memset(encoded->data + sizeof(word) * (encoded->length), 0, lack * sizeof(word));
			encoded->length += lack;
			encoded->capacity = encoded->length;
		}

		bignum_writebin(fp, encoded);

#ifndef NOPRINT
		//printf("%d,%d:", i / bytes, encoded->length);
		//bignum_printhex(stdout, encoded);
		//printf("\n");
#endif
	}
}
 ...
 */
int* decodeMessage(int cryptogram_num, int bytes, bignum* cryptogram, bignum* exponent, bignum* modulus) {
	int* decoded = malloc(cryptogram_num * bytes * sizeof(int));
	int i, j;
	bignum* x = bignum_init(), * remainder = bignum_init();
	bignum* num128 = bignum_init();
	bignum_fromint(num128, 128);
	for (i = 0; i < cryptogram_num; i++) {
...
{
	int blocks = len / encodedBlockSize;
	bignum* cryptogram = bignum_init();
	
	char* decoded = malloc(blockBytes);
	int i, j;
	bignum* x = bignum_init(), * remainder = bignum_init();
	bignum* num128 = bignum_init();
	bignum_fromint(num128, 256);
	for (i = 0; i < blocks; i++) 
	{
		bignum_frombin(cryptogram, message + i * encodedBlockSize, encodedBlockSize);

		decode(cryptogram, exponent, modulus, x);
		for (j = 0; j < blockBytes; j++) 
		{
			bignum_idivider(x, num128, remainder);
			if (remainder->length == 0) 
				decoded[j] = (unsigned char)0;
			else
				decoded[j] = (unsigned char)(remainder->data[0]);
#ifndef NOPRINT
			printf("%c", (unsigned char)(decoded[j]));
#endif
		}
		memcpy_s(result + i * blockBytes, blockBytes, decoded, blockBytes);
	}
	free(decoded);
}
